...
// the JFP2 envelope, per docs/protocol-v2-design.md §6.4. Field order and meaning match the legacy
...
{
    /// <summary>Mirrors the legacy StatusRequest message's three fields exactly (network-protocol.md
    /// §8.6): HubEnabled, HubListRequested, Uuid.</summary>
    public struct StatusRequestUpdate
    {
    ...
    /// <summary>Mirrors the legacy Status reply's fields exactly (network-protocol.md §8.6): node
    /// Guid/AppVersion, session user/ATC/object counts, and - when HubEnabled - the hub directory
    /// metadata block (address, name, about, voip, next event, airport, activity circle, global-
    /// session/password-required flags).</summary>
    public struct StatusUpdate
    {
        ...
        public string AtcAirport; // meaningful only when AtcCount > 0, matching the legacy semantics
        public int AtcLevel;      // int to match Network.Hub.atcLevel's field type; wire is one byte
        public ushort Planes;
        public ushort Helicopters;
        public ushort Boats;
        public ushort Vehicles;
        public bool HubEnabled;
        // Hub directory fields, meaningful only when HubEnabled:
